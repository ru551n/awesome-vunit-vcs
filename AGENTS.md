...
VUNIT_SIMULATOR=nvc python examples/axi4/run.py -p 2 --output-path out/axi4
...
- `src/awesome_vunit_vcs/`: the package. `vhdl/<family>/` holds the VHDL (`common`, `ethernet`,
  `flash`, `i2c`, `axi4`); `common/`, `ethernet/`, `flash/`, `i2c/`, `axi4/`, `records.py` and
  `gen_vhdl.py` hold the Python.
...
- `docs/`: Sphinx. One section per family (`ethernet/`, `property_testing/`, `flash/`, `i2c/`, `axi4/`,
...
- One context clause per family (`ethernet_context`, `flash_context`, `i2c_context`, `axi4_context`,
  `property_context`) must be all a testbench needs.
