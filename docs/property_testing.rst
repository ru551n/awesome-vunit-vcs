...
Python and adds the result each read must give, so the testbench only compares:
...
0xFF. The property finds it and shrinks the lockup to ``[255, 0]``:
...
* Each getter is a call through the Python bridge; ``get_integer_vector`` reads a whole vector in one
  call, and a generated record getter makes one call per scalar field.
