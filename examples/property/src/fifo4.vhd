...
--
-- A depth-4 synchronous FIFO, for the targeted property search of
...
architecture a of fifo4 is
  type memory_t is array (0 to 3) of std_ulogic_vector(7 downto 0);
  signal memory : memory_t := (others => (others => '0'));
  signal wr_ptr, rd_ptr : natural range 0 to 3 := 0;
  signal fill : natural range 0 to 4 := 0;
  signal is_full, is_empty : std_ulogic;
begin
  is_full <= '1' when fill = 4 else '0';
  is_empty <= '1' when fill = 0 else '0';
  full <= is_full;
  empty <= is_empty;
  count <= std_ulogic_vector(to_unsigned(fill, 3));
