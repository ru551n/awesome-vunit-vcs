...
package vc_python_pkg is
  ...
  procedure create_backend(
    session : python_session_t; module_name, class_name : string; args : arg_t := null_arg
  );
  ...
  impure function backend_call_integer(
    session : python_session_t; method : string; args : arg_t := null_arg
  ) return integer;
