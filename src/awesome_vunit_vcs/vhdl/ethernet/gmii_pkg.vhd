...
--
-- The GMII verification components: a source, a monitor and a protocol
-- checker, with handles, constructors and the procedures of ethernet_pkg for
-- them. GMII carries one octet per clock cycle, at 1000 Mbit/s or overclocked
-- at 2500 Mbit/s.
...
package gmii_pkg is
  ...
  -- A GMII protocol checker, for the gmii_protocol_checker entity. It checks the
  -- protocol of one direction of a GMII interface.
  type gmii_protocol_checker_t is record
    -- Private
  ...
  constant null_gmii_protocol_checker : gmii_protocol_checker_t := (
  ...
  constant default_gmii_protocol_checker : gmii_protocol_checker_t := (
  ...
  -- A GMII monitor, for the gmii_monitor entity. It reconstructs the frames of
  -- one direction of a GMII interface.
  type gmii_monitor_t is record
    -- Private
    p_id                         : id_t;
    p_logger                     : logger_t;
    p_actor                      : actor_t;
    ...
    p_unexpected_msg_type_policy : unexpected_msg_type_policy_t;
    p_cfg                        : ethernet_cfg_t;
    p_protocol_checker           : gmii_protocol_checker_t;
  end record;

  -- Create a source.
  --
  -- link_rate_mbps is the rate of the link: 1000, or 2500 for overclocked GMII.
  --
  -- The id defaults to awesome_vunit_vcs:gmii_source:<n>, n counting the
  ...
  ) return gmii_source_t;

  -- Create a monitor.
  --
  -- link_rate_mbps is the rate of the link: 1000, or 2500 for overclocked GMII,
  -- used for utilization statistics.
  ...
  -- default_gmii_protocol_checker for the default checks or a protocol checker
  -- created with :vhdl:`gmii_pkg.new_gmii_protocol_checker`. Its id becomes
  ...
  -- those of :vhdl:`gmii_pkg.new_gmii_source`, with the id defaulting to
  ...
  impure function new_gmii_monitor (
    ...
    protocol_checker : gmii_protocol_checker_t := null_gmii_protocol_checker;
    id : id_t := null_id;
    logger : logger_t := null_logger;
    actor : actor_t := null_actor;
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return gmii_monitor_t;

  -- Create a protocol checker. Violations of its checks are check failures
  -- on its checker.
  --
  -- link_rate_mbps is the rate of the link: 1000, or 2500 for overclocked GMII,
  -- used to measure the IFG.
  ...
  -- batching options are those of :vhdl:`gmii_pkg.new_gmii_monitor`.
  --
  -- The id, logger, actor, checker and unexpected_msg_type_policy are like
  -- those of :vhdl:`gmii_pkg.new_gmii_source`, with the id defaulting to
  -- awesome_vunit_vcs:gmii_protocol_checker:<n>.
  impure function new_gmii_protocol_checker (
    link_rate_mbps : positive := 1000;
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return gmii_protocol_checker_t;

  -- The id of a VC. The Python backend of the VC is the object vc in the
  -- session of this id: ``new_session(get_id(monitor))``.
  impure function get_id (source : gmii_source_t) return id_t;
  impure function get_id (monitor : gmii_monitor_t) return id_t;
  impure function get_id (protocol_checker : gmii_protocol_checker_t) return id_t;

  -- The logger, actor and checker of a VC
  impure function get_logger (source : gmii_source_t) return logger_t;
  impure function get_logger (monitor : gmii_monitor_t) return logger_t;
  impure function get_logger (protocol_checker : gmii_protocol_checker_t) return logger_t;
  impure function get_actor (source : gmii_source_t) return actor_t;
  impure function get_actor (monitor : gmii_monitor_t) return actor_t;
  impure function get_actor (protocol_checker : gmii_protocol_checker_t) return actor_t;
  impure function get_checker (source : gmii_source_t) return checker_t;
  impure function get_checker (monitor : gmii_monitor_t) return checker_t;
  impure function get_checker (protocol_checker : gmii_protocol_checker_t) return checker_t;

  -- The synchronization VCI of a VC
  impure function as_sync (source : gmii_source_t) return sync_handle_t;
  impure function as_sync (monitor : gmii_monitor_t) return sync_handle_t;
  impure function as_sync (protocol_checker : gmii_protocol_checker_t) return sync_handle_t;
  ...
  impure function as_stream (source : gmii_source_t) return stream_master_t;
  impure function as_stream (monitor : gmii_monitor_t) return stream_slave_t;

  -- The Ethernet VCI of a VC
  impure function as_ethernet_source (source : gmii_source_t) return ethernet_source_t;
  impure function as_ethernet_monitor (monitor : gmii_monitor_t) return ethernet_monitor_t;
  impure function as_ethernet_protocol_checker (
    protocol_checker : gmii_protocol_checker_t
  ) return ethernet_protocol_checker_t;

  -- The protocol checker of a monitor, null_gmii_protocol_checker when it has none
  function get_protocol_checker (monitor : gmii_monitor_t) return gmii_protocol_checker_t;

  -- The width of the data port of a VC
  impure function data_length (source : gmii_source_t) return positive;
  impure function data_length (monitor : gmii_monitor_t) return positive;
  impure function data_length (protocol_checker : gmii_protocol_checker_t) return positive;

  -- The procedures of :vhdl:`ethernet_pkg.push_ethernet_frame`,
  -- :vhdl:`ethernet_pkg.push_ethernet_packet` and
  -- :vhdl:`ethernet_pkg.push_ethernet_sequence` for a GMII source
  ...
  -- and :vhdl:`ethernet_pkg.get_statistics`, for a GMII monitor
  ...
  procedure get_frame_count (signal net : inout network_t; monitor : gmii_monitor_t; variable count : out natural);
  procedure log_statistics (signal net : inout network_t; monitor : gmii_monitor_t; log_level : log_level_t := info);
  ...
  procedure stop_capture (signal net : inout network_t; monitor : gmii_monitor_t);

  -- The procedures of :vhdl:`ethernet_pkg.set_check_enabled` and
  -- :vhdl:`ethernet_pkg.get_check_count` for a GMII protocol checker
  ...
  -- (:vhdl:`gmii_pkg.get_protocol_checker`). A protocol check on a monitor
  ...
  procedure reset (signal net : inout network_t; source : gmii_source_t);
  procedure reset (signal net : inout network_t; monitor : gmii_monitor_t; clear_statistics : boolean := false);
  procedure reset (signal net : inout network_t; protocol_checker : gmii_protocol_checker_t);
  ...
  impure function to_ethernet_vc (protocol_checker : gmii_protocol_checker_t) return ethernet_vc_t;
end package;

package body gmii_pkg is

  -- Reports configuration errors of the constructors
  constant gmii_pkg_logger : logger_t := get_logger("awesome_vunit_vcs:gmii_pkg");
  constant gmii_pkg_checker : checker_t := new_checker(gmii_pkg_logger);
  ...
  impure function new_gmii_source (
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return gmii_source_t is

    constant identity : ethernet_identity_t := new_ethernet_identity("gmii_source", id, logger, actor, checker);
  begin
    ...
    return (
      ...
      p_cfg => new_ethernet_cfg(interface => gmii, link_rate_mbps => link_rate_mbps)
    );
  end;
  ...
  impure function new_gmii_protocol_checker (
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return gmii_protocol_checker_t is

    constant identity : ethernet_identity_t :=
      new_ethernet_identity("gmii_protocol_checker", id, logger, actor, checker);
  begin
  ...
  impure function new_gmii_monitor (
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return gmii_monitor_t is

    constant identity : ethernet_identity_t := new_ethernet_identity("gmii_monitor", id, logger, actor, checker);
  ...
  procedure get_frame_count (signal net : inout network_t; monitor : gmii_monitor_t; variable count : out natural) is
  begin

    get_frame_count(net, as_ethernet_monitor(monitor), count);
  end;

  procedure log_statistics (signal net : inout network_t; monitor : gmii_monitor_t; log_level : log_level_t := info) is
  begin
  ...
  procedure reset (signal net : inout network_t; monitor : gmii_monitor_t; clear_statistics : boolean := false) is
  begin
  ...
  impure function to_ethernet_vc (protocol_checker : gmii_protocol_checker_t) return ethernet_vc_t is
  begin
