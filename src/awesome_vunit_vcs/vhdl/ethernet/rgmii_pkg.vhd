...
--
-- The RGMII verification components: a source, a monitor and a protocol
-- checker, with handles, constructors and the procedures of ethernet_pkg for
-- them. RGMII carries the lower 4 bits of an octet on the rising and the
-- upper 4 bits on the falling clock edge at 1000 Mbit/s (125 MHz clock), and
-- one nibble per clock cycle at 100 and 10 Mbit/s (25 and 2.5 MHz). CTL is the
...
package rgmii_pkg is
  ...
  -- How data lines up with the clock. rgmii_centered: data is stable around
  -- each clock edge, like the output of an RGMII-ID transmitter, which adds the
  -- clock delay internally. A source changes data on the opposite clock edge and
  -- a monitor samples on the clock edges. rgmii_edge_aligned: data changes
  -- together with the clock edges, like the output of a DUT that registers its
  -- outputs on the clock. A source changes data on the clock edges and a monitor
  -- samples a quarter clock period after them.
  type rgmii_data_timing_t is (rgmii_centered, rgmii_edge_aligned);

  -- An RGMII source, for the rgmii_source entity. It drives one direction of a RGMII
  -- interface.
  ...
  constant null_rgmii_protocol_checker : rgmii_protocol_checker_t := (
  ...
  -- :vhdl:`rgmii_pkg.new_rgmii_protocol_checker`
  constant default_rgmii_protocol_checker : rgmii_protocol_checker_t := (
  ...
  -- An RGMII monitor, for the rgmii_monitor entity. It reconstructs the frames of
  ...
  -- Create a source.
  --
  -- link_rate_mbps is the rate of the link, 10, 100 or 1000. data_timing is
  -- rgmii_centered when data is stable around each clock edge, like with an
  -- RGMII-ID transmitter, and rgmii_edge_aligned when data changes together
  -- with the clock edges.
  --
  -- The id defaults to awesome_vunit_vcs:rgmii_source:<n>, n counting the
  ...
  ) return rgmii_source_t;

  -- Create a monitor.
  --
  -- link_rate_mbps is the rate of the link, 10, 100 or 1000, used for utilization
  -- statistics. data_timing is that of :vhdl:`rgmii_pkg.new_rgmii_source`.
  ...
  -- default_rgmii_protocol_checker for the default checks or a protocol checker
  -- created with :vhdl:`rgmii_pkg.new_rgmii_protocol_checker`. Its id becomes
  ...
  impure function new_rgmii_monitor(
    ...
    protocol_checker : rgmii_protocol_checker_t := null_rgmii_protocol_checker;
    id : id_t := null_id;
    logger : logger_t := null_logger;
    actor : actor_t := null_actor;
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return rgmii_monitor_t;

  -- Create a protocol checker. Violations of its checks are check failures
  -- on its checker.
  --
  -- link_rate_mbps is the rate of the link, 10, 100 or 1000, used to measure the IFG.
  -- data_timing is that of :vhdl:`rgmii_pkg.new_rgmii_source`.
  ...
  -- batching options are those of :vhdl:`rgmii_pkg.new_rgmii_monitor`.
  --
  -- The id, logger, actor, checker and unexpected_msg_type_policy are like
  -- those of :vhdl:`rgmii_pkg.new_rgmii_source`, with the id defaulting to
  ...
  impure function new_rgmii_protocol_checker(
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return rgmii_protocol_checker_t;
  ...
  begin
    if protocol_checker.p_type = null_ethernet_component then
      ...
      result.p_cfg := new_ethernet_cfg(
        ...
        edge_aligned => monitor_cfg.p_edge_aligned,
        min_ifg_octets => 12,
