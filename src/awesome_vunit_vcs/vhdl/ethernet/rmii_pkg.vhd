...
--
-- The RMII verification components: a source, a monitor and a protocol
-- checker, with handles, constructors and the procedures of ethernet_pkg for
-- them. RMII carries two bits, a dibit, per cycle of a 50 MHz reference clock,
-- least significant bits of an octet first, at 100 Mbit/s, and every dibit for
...
package rmii_pkg is
  ...
  -- An RMII source, for the rmii_source entity. It drives one direction of a RMII
  -- interface.
  ...
  constant null_rmii_protocol_checker : rmii_protocol_checker_t := (
  ...
  -- :vhdl:`rmii_pkg.new_rmii_protocol_checker`
  constant default_rmii_protocol_checker : rmii_protocol_checker_t := (
  ...
  -- An RMII monitor, for the rmii_monitor entity. It reconstructs the frames of
  ...
  -- Create a source.
  --
  -- link_rate_mbps is the rate of the link, 10 or 100. With crs_dv_toggle_octets
  -- above 0, the source transmits like a PHY whose carrier ends before its data:
  -- dv toggles, low on the first and high on the second dibit of every nibble,
  -- during the last crs_dv_toggle_octets octets of each frame.
  --
  -- The id defaults to awesome_vunit_vcs:rmii_source:<n>, n counting the
  ...
  -- default_rmii_protocol_checker for the default checks or a protocol checker
  -- created with :vhdl:`rmii_pkg.new_rmii_protocol_checker`. Its id becomes
  ...
  impure function new_rmii_monitor(
    ...
    protocol_checker : rmii_protocol_checker_t := null_rmii_protocol_checker;
    id : id_t := null_id;
    logger : logger_t := null_logger;
    actor : actor_t := null_actor;
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return rmii_monitor_t;
  ...
  -- batching options are those of :vhdl:`rmii_pkg.new_rmii_monitor`.
  --
  -- The id, logger, actor, checker and unexpected_msg_type_policy are like
  -- those of :vhdl:`rmii_pkg.new_rmii_source`, with the id defaulting to
  ...
  impure function new_rmii_protocol_checker(
    ...
    checker : checker_t := null_checker;
    unexpected_msg_type_policy : unexpected_msg_type_policy_t := fail
  ) return rmii_protocol_checker_t;
  ...
  -- instantiates (:vhdl:`rmii_pkg.get_protocol_checker`). A monitor without a
