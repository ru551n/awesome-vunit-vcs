...
from awesome_vunit_vcs.i2c import CheckId, I2cProtocolChecker, I2cValueError, SpeedMode, Violation, bus_limits
...
def _violations(bus: Bus, mode: SpeedMode = SpeedMode.FAST, **kwargs: int) -> list[Violation]:
...
def _checks(bus: Bus, mode: SpeedMode = SpeedMode.FAST) -> list[CheckId]:
...
def test_t_low() -> None:
    ...
    assert {violation.check for violation in violations} == {CheckId.T_LOW}
...
def test_t_high_and_f_scl() -> None:
    ...
    assert set(_checks(_bus(t_high=500 * NS).write(0x50))) == {CheckId.T_HIGH, CheckId.F_SCL}
...
def test_f_scl_alone() -> None:
    ...
    assert set(_checks(bus.write(0x50), SpeedMode.STANDARD)) == {CheckId.F_SCL}


def test_start_and_stop_setup_and_hold() -> None:
    assert set(_checks(_bus(t_hd_sta=400 * NS).write(0x50))) == {CheckId.T_HD_STA}
    assert set(_checks(_bus(t_su_sto=400 * NS).write(0x50))) == {CheckId.T_SU_STO}
    bus = _bus(t_su_sta=400 * NS).start().byte(0xA0).start().byte(0xA1, ack=False).stop()
    assert _checks(bus) == [CheckId.T_SU_STA]


def test_t_buf() -> None:
    assert _checks(_bus(t_buf=1000 * NS).write(0x50).write(0x50)) == [CheckId.T_BUF]


def test_data_setup_and_hold() -> None:
    # SDA changes 1250 ns into a 1300 ns low period: 50 ns setup
    assert set(_checks(_bus(t_hd_dat=1250 * NS).write(0x50, b"\x55"))) == {CheckId.T_SU_DAT}
    limits = bus_limits(SpeedMode.FAST, t_hd_dat_fs=400 * NS)
    checker = I2cProtocolChecker(limits)
    bus = _bus().write(0x50, b"\x55")
    checker.feed(bus.words, bus.times)
    assert checker.count(CheckId.T_HD_DAT) > 0
...
def test_a_stop_inside_a_byte_and_a_byte_without_acknowledge() -> None:
    ...
    for bit in (1, 0, 1):
        bus.bit(bit)
    assert _checks(bus.stop()) == [CheckId.SDA_STABLE]
    bus = _bus().start().byte(0xA0)
    for bit in (1, 0, 1, 0, 1, 0, 1, 0):
        bus.bit(bit)
    assert _checks(bus.start().byte(0xA1, ack=False).stop()) == [CheckId.ACK_SLOT]


def test_metavalue_and_stuck_low() -> None:
    bus = _bus().write(0x50).metavalue("sda").metavalue("sda", after=10 * NS).set(sda=1, after=10 * NS)
    assert _checks(bus) == [CheckId.METAVALUE]
    ...
    assert [violation.check for violation in violations] == [CheckId.STUCK_LOW]
...
def test_enable_disable_count_and_reset() -> None:
    ...
    checker.feed(bus.words, bus.times)
    assert checker.count(CheckId.T_BUF) == 0 and not checker.is_enabled("T_BUF")
    checker.enable(CheckId.T_BUF)
    checker.reset()
    checker.feed(bus.words, bus.times)
    assert checker.count("I2C_T_BUF") == 2
    checker.reset()
    assert checker.count(CheckId.T_BUF) == 0
    with pytest.raises(I2cValueError):
        checker.count(CheckId.SCOREBOARD)
    with pytest.raises(I2cValueError):
