...
--
-- GMII source and monitor: a source drives a GMII line that two monitors
-- observe. Every frame a test sends is reconstructed and checked in Python by
-- both monitors. A test expecting violations counts them on both monitors.
...
architecture tb of tb_gmii is
  ...
  constant second_monitor : ethernet_monitor_t := new_gmii_monitor;
...
begin
  ...
  main : process
    ...
    -- Check that each monitor found exactly expected violations of check and
    -- that they were the only errors logged
  ...
  begin
    ...
    while test_suite loop
      if run("test_minimum_size_frame") then
        ...
        send_ethernet_frame(net, source, frame_data(60), preamble_octets => 5);
        check_violations(eth_preamble, 1);

        send_ethernet_frame(net, source, frame_data(60), sfd => x"D4");
        check_violations(eth_sfd, 1);

      elsif run("test_fcs_pass") then
        for idx in 1 to 10 loop
          send_expected_frame(frame_data(rnd.RandInt(60, 1514), seed => idx));
        end loop;
        ...
        send_ethernet_frame(net, source, frame_data(20), pad => false);
        check_violations(eth_runt, 1);
        get_statistics(net, monitor, statistics);
        check_equal(statistics.runts, 1);

      elsif run("test_oversized_frame") then
        send_ethernet_frame(net, source, frame_data(1600));
        ...
        for idx in 1 to 5 loop
          send_expected_frame(frame_data(100, seed => idx), ifg_octets => 12);
        end loop;
        wait_until_idle;
        for idx in monitors'range loop
        ...
        -- The same line, but only the first monitor checks the FCS and only
        -- the second expects a frame
