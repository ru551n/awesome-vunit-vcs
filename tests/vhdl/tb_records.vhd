  ...
  begin
    ...
    while test_suite loop
      if run("test_generated_records_read_every_field_type") then
        prop := new_property("record_types:links", max_examples => 200, seed => get_seed(runner_cfg),
        ...
        prop := new_property("record_types:lanes_only", max_examples => 20, seed => get_seed(runner_cfg),
          search_path => tb_path(runner_cfg) & "python");
        while next_example(prop) loop
          lane := get_lane(prop);
          report_example(prop, passed => lane.index = get_integer(prop, "index") and lane.enabled = get_boolean(prop, "enabled"));
        end loop;
        check_property(prop);
      end if;
    end loop;
