...
architecture tb of tb_rgmii is
  ...
  constant monitor : rgmii_monitor_t := new_rgmii_monitor(
    link_rate_mbps => link_rate_mbps, data_timing => data_timing,
    protocol_checker => default_rgmii_protocol_checker, id => get_id("tb_rgmii:monitor")
  );
  constant second_monitor : rgmii_monitor_t := new_rgmii_monitor(
    link_rate_mbps => link_rate_mbps, data_timing => data_timing,
    protocol_checker => default_rgmii_protocol_checker, id => get_id("tb_rgmii:second_monitor")
  );
...
begin
  ...
  main : process
    ...
    begin
      if gigabit then
        return
          kwarg("count", 60) & kwarg("malformations", common & ",bad_sfd") &
          kwarg("malformed_fraction", 0.3) & kwarg("interface", "rgmii");
      end if;
      return
        kwarg("count", 60) & kwarg("malformations", common) &
        kwarg("malformed_fraction", 0.3) & kwarg("interface", "rgmii");
    end;
