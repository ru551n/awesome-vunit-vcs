...
--
-- Conformance of the RGMII source, monitor and protocol checker with the VUnit
...
entity tb_rgmii_vci is
  generic (
    runner_cfg : string
  );
end entity;

architecture tb of tb_rgmii_vci is
  constant clk_period : time := 8 ns;
  signal clk_running : boolean := true;
  signal clk : std_ulogic := '0';

  -- The line the source drives and every monitor and protocol checker observes
  signal data : std_ulogic_vector(3 downto 0);
  signal ctl : std_ulogic;
  -- The lines of the other sources, observed by nobody
  signal second_data, ignoring_data : std_ulogic_vector(3 downto 0);
  signal second_ctl, ignoring_ctl : std_ulogic;

  constant source : rgmii_source_t := new_rgmii_source;
  constant second_source : rgmii_source_t := new_rgmii_source;
  constant ignoring_source : rgmii_source_t := new_rgmii_source(unexpected_msg_type_policy => ignore);

  constant monitor : rgmii_monitor_t := new_rgmii_monitor(
    protocol_checker => default_rgmii_protocol_checker, id => get_id("tb_rgmii_vci:monitor")
  );
  constant default_monitor : rgmii_monitor_t := new_rgmii_monitor;
  constant second_default_monitor : rgmii_monitor_t := new_rgmii_monitor;
  constant ignoring_monitor : rgmii_monitor_t := new_rgmii_monitor(unexpected_msg_type_policy => ignore);

  constant protocol_checker : rgmii_protocol_checker_t := new_rgmii_protocol_checker;
  constant second_protocol_checker : rgmii_protocol_checker_t := new_rgmii_protocol_checker;
  constant ignoring_protocol_checker : rgmii_protocol_checker_t := new_rgmii_protocol_checker(
    unexpected_msg_type_policy => ignore
  );

  type rgmii_protocol_checker_vec_t is array (natural range <>) of rgmii_protocol_checker_t;
  -- Every protocol checker observing the line
  constant protocol_checkers : rgmii_protocol_checker_vec_t := (
  ...
  constant subscriber : actor_t := new_actor("tb_rgmii_vci:subscriber");
begin
  ...
  main : process
    ...
    variable custom_source : rgmii_source_t;
    variable custom_monitor : rgmii_monitor_t;
    ...
    while test_suite loop
      if run("test_default_ids_are_enumerated_under_the_vc_name") then
        check_default_identity(
          get_id(source), get_logger(source), get_actor(source), get_checker(source), "rgmii_source"
        );
        check_default_identity(
          get_id(default_monitor), get_logger(default_monitor), get_actor(default_monitor),
          get_checker(default_monitor), "rgmii_monitor"
        );
        check_default_identity(
          get_id(protocol_checker), get_logger(protocol_checker), get_actor(protocol_checker),
          get_checker(protocol_checker), "rgmii_protocol_checker"
        );
        ...
        check_equal(full_name(get_id(monitor)), "tb_rgmii_vci:monitor");
        ...
        check_equal(full_name(get_id(get_protocol_checker(monitor))), "tb_rgmii_vci:monitor:protocol_checker");
        check_equal(get_full_name(get_logger(get_protocol_checker(monitor))), "tb_rgmii_vci:monitor:protocol_checker");
        check(get_id(get_actor(get_protocol_checker(monitor))) = get_id(get_protocol_checker(monitor)));
        check(get_protocol_checker(default_monitor) = null_rgmii_protocol_checker);

      elsif run("test_explicit_logger_actor_and_checker_are_kept") then
        custom_logger := get_logger("tb_rgmii_vci:custom");
        custom_actor := new_actor("tb_rgmii_vci:custom actor");
        custom_checker := new_checker("tb_rgmii_vci:custom checker");
        custom_source := new_rgmii_source(logger => custom_logger, actor => custom_actor, checker => custom_checker);
        ...
        custom_monitor := new_rgmii_monitor(protocol_checker => new_rgmii_protocol_checker(logger => custom_logger));
        ...
        wait until rising_edge(clk) and ctl = '1';
        ...
        check_equal(data'length, data_length(source));
        check_equal(data_length(monitor), 4);
        check_equal(data_length(protocol_checker), 4);
      end if;
    end loop;
  ...
  second_source_inst : entity awesome_vunit_vcs.rgmii_source
    generic map (
  ...
  ignoring_source_inst : entity awesome_vunit_vcs.rgmii_source
    generic map (
  ...
  default_monitor_inst : entity awesome_vunit_vcs.rgmii_monitor
    generic map (
  ...
  second_default_monitor_inst : entity awesome_vunit_vcs.rgmii_monitor
    generic map (
  ...
  ignoring_monitor_inst : entity awesome_vunit_vcs.rgmii_monitor
    generic map (
  ...
  protocol_checker_inst : entity awesome_vunit_vcs.rgmii_protocol_checker
    generic map (
  ...
  second_protocol_checker_inst : entity awesome_vunit_vcs.rgmii_protocol_checker
    generic map (
  ...
  ignoring_protocol_checker_inst : entity awesome_vunit_vcs.rgmii_protocol_checker
    generic map (
