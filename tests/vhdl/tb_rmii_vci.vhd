...
--
-- Conformance of the RMII source, monitor and protocol checker with the VUnit
...
entity tb_rmii_vci is
  generic (
    runner_cfg : string
  );
end entity;

architecture tb of tb_rmii_vci is
  ...
  signal second_data, ignoring_data : std_ulogic_vector(1 downto 0);
  signal second_dv, second_er, ignoring_dv, ignoring_er : std_ulogic;

  constant source : rmii_source_t := new_rmii_source;
  constant second_source : rmii_source_t := new_rmii_source;
  constant ignoring_source : rmii_source_t := new_rmii_source(unexpected_msg_type_policy => ignore);

  constant monitor : rmii_monitor_t := new_rmii_monitor(
    protocol_checker => default_rmii_protocol_checker, id => get_id("tb_rmii_vci:monitor")
  );
  constant default_monitor : rmii_monitor_t := new_rmii_monitor;
  constant second_default_monitor : rmii_monitor_t := new_rmii_monitor;
  constant ignoring_monitor : rmii_monitor_t := new_rmii_monitor(unexpected_msg_type_policy => ignore);

  constant protocol_checker : rmii_protocol_checker_t := new_rmii_protocol_checker;
  constant second_protocol_checker : rmii_protocol_checker_t := new_rmii_protocol_checker;
  constant ignoring_protocol_checker : rmii_protocol_checker_t := new_rmii_protocol_checker(
    unexpected_msg_type_policy => ignore
  );

  type rmii_protocol_checker_vec_t is array (natural range <>) of rmii_protocol_checker_t;
  -- Every protocol checker observing the line
  constant protocol_checkers : rmii_protocol_checker_vec_t := (
  ...
  constant subscriber : actor_t := new_actor("tb_rmii_vci:subscriber");
begin
  ...
  main : process
    ...
    variable custom_source : rmii_source_t;
    variable custom_monitor : rmii_monitor_t;
    ...
    while test_suite loop
      if run("test_default_ids_are_enumerated_under_the_vc_name") then
        check_default_identity(
          get_id(source), get_logger(source), get_actor(source), get_checker(source), "rmii_source"
        );
        check_default_identity(
          get_id(default_monitor), get_logger(default_monitor), get_actor(default_monitor),
          get_checker(default_monitor), "rmii_monitor"
        );
        check_default_identity(
          get_id(protocol_checker), get_logger(protocol_checker), get_actor(protocol_checker),
          get_checker(protocol_checker), "rmii_protocol_checker"
        );
        ...
        check_equal(full_name(get_id(monitor)), "tb_rmii_vci:monitor");
        ...
        check_equal(full_name(get_id(get_protocol_checker(monitor))), "tb_rmii_vci:monitor:protocol_checker");
        check_equal(get_full_name(get_logger(get_protocol_checker(monitor))), "tb_rmii_vci:monitor:protocol_checker");
        check(get_id(get_actor(get_protocol_checker(monitor))) = get_id(get_protocol_checker(monitor)));
        check(get_protocol_checker(default_monitor) = null_rmii_protocol_checker);

      elsif run("test_explicit_logger_actor_and_checker_are_kept") then
        custom_logger := get_logger("tb_rmii_vci:custom");
        custom_actor := new_actor("tb_rmii_vci:custom actor");
        custom_checker := new_checker("tb_rmii_vci:custom checker");
        custom_source := new_rmii_source(logger => custom_logger, actor => custom_actor, checker => custom_checker);
        ...
        custom_monitor := new_rmii_monitor(protocol_checker => new_rmii_protocol_checker(logger => custom_logger));
        ...
        check_equal(data_length(monitor), 2);
        check_equal(data_length(protocol_checker), 2);
      end if;
    end loop;
  ...
  second_source_inst : entity awesome_vunit_vcs.rmii_source
    generic map (
  ...
  ignoring_source_inst : entity awesome_vunit_vcs.rmii_source
    generic map (
  ...
  default_monitor_inst : entity awesome_vunit_vcs.rmii_monitor
    generic map (
  ...
  second_default_monitor_inst : entity awesome_vunit_vcs.rmii_monitor
    generic map (
  ...
  ignoring_monitor_inst : entity awesome_vunit_vcs.rmii_monitor
    generic map (
  ...
  protocol_checker_inst : entity awesome_vunit_vcs.rmii_protocol_checker
    generic map (
  ...
  second_protocol_checker_inst : entity awesome_vunit_vcs.rmii_protocol_checker
    generic map (
  ...
  ignoring_protocol_checker_inst : entity awesome_vunit_vcs.rmii_protocol_checker
    generic map (
