...
--
-- Conformance of the XGMII source, monitor and protocol checker with the VUnit
...
architecture tb of tb_xgmii_vci is
  ...
  signal second_data, ignoring_data : std_ulogic_vector(31 downto 0);
  signal second_ctrl, ignoring_ctrl : std_ulogic_vector(3 downto 0);

  constant source : xgmii_source_t := new_xgmii_source;
...
begin
  ...
  main : process
    ...
    variable custom_source : xgmii_source_t;
  ...
  begin
    ...
    while test_suite loop
      ...
      if run("test_default_ids_are_enumerated_under_the_vc_name") then
        ...
        check_equal(full_name(get_id(monitor)), "tb_xgmii_vci:monitor");
        ...
        check_equal(full_name(get_id(get_protocol_checker(monitor))), "tb_xgmii_vci:monitor:protocol_checker");
        check_equal(get_full_name(get_logger(get_protocol_checker(monitor))), "tb_xgmii_vci:monitor:protocol_checker");
        check(get_id(get_actor(get_protocol_checker(monitor))) = get_id(get_protocol_checker(monitor)));
        check(get_protocol_checker(default_monitor) = null_xgmii_protocol_checker);

      elsif run("test_explicit_logger_actor_and_checker_are_kept") then
        custom_logger := get_logger("tb_xgmii_vci:custom");
        custom_actor := new_actor("tb_xgmii_vci:custom actor");
        custom_checker := new_checker("tb_xgmii_vci:custom checker");
        custom_source := new_xgmii_source(logger => custom_logger, actor => custom_actor, checker => custom_checker);
  ...
  second_source_inst : entity awesome_vunit_vcs.xgmii_source
    generic map (
  ...
  ignoring_source_inst : entity awesome_vunit_vcs.xgmii_source
    generic map (
  ...
  default_monitor_inst : entity awesome_vunit_vcs.xgmii_monitor
    generic map (
  ...
  second_default_monitor_inst : entity awesome_vunit_vcs.xgmii_monitor
    generic map (
  ...
  ignoring_monitor_inst : entity awesome_vunit_vcs.xgmii_monitor
    generic map (
  ...
  protocol_checker_inst : entity awesome_vunit_vcs.xgmii_protocol_checker
    generic map (
  ...
  second_protocol_checker_inst : entity awesome_vunit_vcs.xgmii_protocol_checker
    generic map (
  ...
  ignoring_protocol_checker_inst : entity awesome_vunit_vcs.xgmii_protocol_checker
    generic map (
