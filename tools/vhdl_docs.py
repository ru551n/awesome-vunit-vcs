...
def generate(vhdl_root: Path, output: Path, *, source_root: Path | None = None) -> list[Path]:
    """Write ``<output>/<family>.inc`` for every family directory, and ``<family>.<group>.inc`` per group.

    Returns the family files.
    """
    ...
    for family_dir in sorted(path for path in vhdl_root.iterdir() if path.is_dir()):
        ...
        for path in files:
            ...
            for unit in parse_file(path):
                rendered = [*render_unit(unit, relative), ""]
                out.extend(rendered)
                groups.setdefault(group, []).extend(rendered)
        written.append(_write(output / f"{family}.inc", out))
        # One file per group as well, so a documentation section can include a single group
        for group, lines in groups.items():
            _write(output / f"{family}.{group}.inc", [out[0], "", *lines])
    return written
